      ...
      <div className="min-h-screen" style={{ backgroundColor: 'var(--background)' }}>
        ...
        >
          <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {['What It Is', 'Features', 'How It Works', 'Demo', 'FAQ'].map((item) => (
                <button
                  key={item}
            ...
            <div className="hidden lg:flex items-center gap-4">
              <a href="#" className="text-white font-medium px-4 py-2">Login</a>
              <a
                href="#"
                className="bg-white px-5 py-2.5 rounded-lg font-semibold transition-all hover:-translate-y-0.5 hover:shadow-lg"
          ...
          {/* Mobile Menu */}
          {mobileMenuOpen && (
            <div className="lg:hidden px-6 py-4 space-y-2" style={{ backgroundColor: 'var(--primary)' }}>
              {['What It Is', 'Features', 'How It Works', 'Demo', 'FAQ'].map((item) => (
                <button
                  key={item}
                  onClick={() => scrollToSection(item.toLowerCase().replace(/\s+/g, '-'))}
                  className="block w-full text-left text-white py-2 px-4 rounded-lg hover:bg-white/10"
                >
                  {item}
                </button>
              ))}
              <a href="#" className="block text-white py-2 px-4">Login</a>
              <a href="#" className="block bg-white text-center py-2 px-4 rounded-lg font-semibold" style={{ color: 'var(--primary)' }}>
                Get Started
              </a>
            </div>
          )}
        ...
        <section className="min-h-screen flex items-center pt-32 pb-20 px-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #041c2c 0%, #062d47 50%, #041c2c 100%)' }}>
          ...
          <div className="max-w-5xl mx-auto text-center relative z-10">
            ...
            <p className="animate-on-scroll stagger-2 text-lg md:text-xl text-white/70 max-w-2xl mx-auto mb-10 leading-relaxed">
              Monitor, control, and optimize your HVAC units from anywhere. Real-time data, intelligent predictions, and seamless automation powered by cutting-edge IoT technology.
            </p>

            <div className="animate-on-scroll stagger-3 flex flex-col sm:flex-row justify-center gap-4 mb-16">
              <a
                href="#"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-white transition-all hover:-translate-y-1"
                style={{ background: 'linear-gradient(135deg, var(--primary), var(--primary-dark))', boxShadow: '0 4px 20px rgba(9, 65, 102, 0.4)' }}
              >
                Get Started Free
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-white bg-white/10 border border-white/30 backdrop-blur-sm hover:bg-white/20 transition-all hover:-translate-y-1"
              >
            ...
            <div className="animate-on-scroll stagger-4 flex flex-wrap justify-center gap-8 md:gap-16">
              {[
        ...
        <section className="py-24 px-6 relative overflow-hidden" style={{ background: 'linear-gradient(180deg, #0a1929 0%, #0d2137 100%)' }}>
          <div className="max-w-7xl mx-auto">
            <SectionHeader
              label="THE PROBLEM"
              title="Traditional HVAC Management Is"
              titleHighlight="Broken"
              highlightColor="#ef4444"
              description="Building operators face critical challenges that cost time, money, and comfort"
              dark
            />

            <div className="animate-on-scroll stagger-3 relative flex justify-center items-center mb-16 min-h-[350px]">
              {[300, 220, 140].map((size, i) => (
                <div
                  key={size}
                  className={`absolute rounded-full border border-dashed border-white/20 ${i === 1 ? 'rotating-circle-reverse' : 'rotating-circle'}`}
                  style={{ width: size, height: size, animationDuration: `${30 - i * 5}s` }}
                />
              ))}
              <div
                className="w-24 h-24 rounded-3xl flex items-center justify-center z-10"
                style={{ background: 'linear-gradient(135deg, var(--primary), var(--accent))', boxShadow: '0 10px 40px rgba(9, 65, 102, 0.4)' }}
              >
                <AlertTriangleIcon className="w-10 h-10 text-white" />
              </div>

              {/* Problem Points - Hidden on mobile */}
              <div className="hidden lg:block absolute top-[10%] left-[5%] bg-white/5 border border-white/10 p-4 rounded-xl backdrop-blur-sm max-w-[200px]">
                <h4 className="text-white font-semibold text-sm mb-1">Blind Spots</h4>
                <p className="text-white/60 text-xs">No real-time visibility</p>
              </div>
              <div className="hidden lg:block absolute top-[10%] right-[5%] bg-white/5 border border-white/10 p-4 rounded-xl backdrop-blur-sm max-w-[200px]">
                <h4 className="text-white font-semibold text-sm mb-1">Energy Waste</h4>
                <p className="text-white/60 text-xs">40% over-consumption</p>
              </div>
              <div className="hidden lg:block absolute bottom-[10%] left-[10%] bg-white/5 border border-white/10 p-4 rounded-xl backdrop-blur-sm max-w-[200px]">
                <h4 className="text-white font-semibold text-sm mb-1">Reactive Only</h4>
                <p className="text-white/60 text-xs">Fix after failure</p>
              </div>
              <div className="hidden lg:block absolute bottom-[10%] right-[10%] bg-white/5 border border-white/10 p-4 rounded-xl backdrop-blur-sm max-w-[200px]">
                <h4 className="text-white font-semibold text-sm mb-1">Manual Work</h4>
                <p className="text-white/60 text-xs">Time-consuming checks</p>
              </div>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { icon: <EyeIcon className="w-8 h-8" />, title: 'Lack of Visibility', desc: 'No real-time monitoring means issues go undetected until they become major problems.' },
                { icon: <BoltIcon className="w-8 h-8" />, title: 'Energy Inefficiency', desc: 'Traditional systems waste up to 40% of energy due to poor optimization.' },
                { icon: <ClockIcon className="w-8 h-8" />, title: 'Reactive Maintenance', desc: 'Fixing problems after they occur leads to costly downtime and repairs.' },
                { icon: <UsersIcon className="w-8 h-8" />, title: 'Manual Operations', desc: 'Staff spend hours on routine checks that could be automated.' },
              ].map((problem, i) => (
                <div
                  key={problem.title}
                  className={`animate-on-scroll stagger-${i + 1} bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 transition-all hover:bg-white/[0.06] hover:-translate-y-1`}
                >
        ...
        <section id="features" className="py-24 px-6" style={{ background: 'linear-gradient(180deg, #f9fafb 0%, white 100%)' }}>
          <div className="max-w-7xl mx-auto">
            <SectionHeader
              label="KEY FEATURES"
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
                { icon: <DashboardIcon className="w-7 h-7 text-white" />, bg: 'linear-gradient(135deg, #0c5a8a, #094166)', title: 'Real-time Monitoring', desc: 'Track temperature, humidity, energy usage, and system status across all your units in real-time with live dashboards.', tag: 'Core Feature' },
                { icon: <CogIcon className="w-7 h-7 text-white" />, bg: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', title: 'Remote Control', desc: 'Adjust temperature, fan speed, and operating modes from anywhere using your smartphone, tablet, or computer.', tag: 'Mobile Ready' },
                { icon: <BellIcon className="w-7 h-7 text-white" />, bg: 'linear-gradient(135deg, #06b6d4, #0891b2)', title: 'Smart Alerts', desc: 'Get instant notifications for faults, maintenance needs, and anomalies before they become costly problems.', tag: 'Proactive' },
                { icon: <ChartIcon className="w-7 h-7 text-white" />, bg: 'linear-gradient(135deg, #10b981, #059669)', title: 'Energy Analytics', desc: 'Analyze consumption patterns, identify waste, and optimize efficiency with detailed energy reports and insights.', tag: 'Cost Saving' },
                { icon: <CalendarIcon className="w-7 h-7 text-white" />, bg: 'linear-gradient(135deg, #f59e0b, #d97706)', title: 'Smart Scheduling', desc: 'Set up automated schedules based on occupancy, time of day, or custom rules for optimal comfort and efficiency.', tag: 'Automation' },
                { icon: <ShieldIcon className="w-7 h-7 text-white" />, bg: 'linear-gradient(135deg, #ef4444, #dc2626)', title: 'Enterprise Security', desc: 'Bank-level encryption, JWT authentication, and two-factor authentication keep your systems secure.', tag: 'Secure' },
              ].map((feature, i) => (
                <div
                  key={feature.title}
                  className={`animate-on-scroll animate-scale stagger-${i + 1} bg-white p-8 rounded-3xl border border-gray-100 relative overflow-hidden transition-all duration-400 hover:-translate-y-3 hover:shadow-2xl group`}
                  style={{ transformStyle: 'preserve-3d' }}
                >
                  <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[var(--primary)] to-cyan-500 transform scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-400" />
                  <div
                    className="w-[72px] h-[72px] rounded-2xl flex items-center justify-center mb-6 transition-transform duration-400 group-hover:scale-110 group-hover:rotate-[5deg]"
                  ...
                  <span className="inline-block px-4 py-1.5 bg-gray-100 text-gray-600 rounded-full text-xs font-medium">
                    {feature.tag}
                  </span>
                </div>
              ))}
...
<section id="how-it-works" className="py-24 px-6 bg-white overflow-hidden">
  <div className="max-w-7xl mx-auto">
    ...
    <div className="mt-20 relative">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-sky-50 rounded-full blur-3xl opacity-30 -z-10" />

      <div className="max-w-5xl mx-auto space-y-8">
        {[
          { num: '01', icon: <UserIcon className="w-7 h-7" />, title: 'Create Your Account', desc: 'Sign up for free in seconds. Set up your organization profile and invite your team members to collaborate.', label: 'Setup' },
          { num: '02', icon: <PlugIcon className="w-7 h-7" />, title: 'Connect Your Devices', desc: 'Add your HVAC units using our simple pairing process. We support all major brands and communication protocols.', label: 'Integration' },
          { num: '03', icon: <CogsIcon className="w-7 h-7" />, title: 'Configure Settings', desc: 'Set up your preferences, create schedules, define alert thresholds, and customize your dashboard layout.', label: 'Customization' },
          { num: '04', icon: <RocketIcon className="w-7 h-7" />, title: 'Start Optimizing', desc: 'Monitor your systems, receive intelligent insights, and watch your energy efficiency improve automatically.', label: 'Launch' },
        ].map((step, i) => (
          <div 
            key={step.num}
            className="group relative flex flex-col md:flex-row gap-6 md:gap-12 p-8 md:p-10 rounded-[2rem] border border-gray-100 bg-white/80 backdrop-blur-sm transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:border-[var(--primary)]/20"
          >
            {/* Step Number & Indicator */}
            <div className="flex flex-row md:flex-col items-center gap-4">
              <div 
                className="w-16 h-16 shrink-0 rounded-2xl flex items-center justify-center text-2xl font-black transition-all duration-500 group-hover:scale-110 shadow-sm"
                style={{ backgroundColor: 'var(--primary)', color: 'white' }}
              >
                {step.num}
              </div>
              <div className="h-px md:h-20 w-full md:w-px bg-gradient-to-b from-[var(--primary)] to-transparent opacity-20 group-last:hidden" />
            </div>

            {/* Content Body */}
            <div className="flex-grow">
              <div className="flex items-center gap-3 mb-4">
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase py-1 px-3 rounded-full bg-gray-100 text-gray-500 group-hover:bg-[var(--primary)] group-hover:text-white transition-colors duration-300">
                  {step.label}
                </span>
            ...
            {/* Floating Icon Decor */}
            <div 
              className="absolute right-10 top-1/2 -translate-y-1/2 opacity-[0.03] group-hover:opacity-10 transition-opacity duration-500 hidden lg:block"
              style={{ color: 'var(--primary)' }}
            >
        ...
        <section className="py-24 px-6 bg-white">
          <div className="max-w-7xl mx-auto">
            <SectionHeader
              ...
              description="Join thousands of organizations already seeing measurable results"
            />

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
                { stat: '30%', title: 'Energy Reduction', desc: 'Average energy savings achieved by our customers within the first 6 months' },
                { stat: '50%', title: 'Less Downtime', desc: 'Reduction in unplanned system failures through predictive maintenance' },
                { stat: '40%', title: 'Labor Savings', desc: 'Decrease in manual monitoring tasks through intelligent automation' },
                { stat: '99.9%', title: 'System Uptime', desc: 'Platform reliability backed by our enterprise SLA guarantee' },
                { stat: '3x', title: 'Faster Response', desc: 'Response time to critical alerts with real-time notifications' },
                { stat: '$50K+', title: 'Annual Savings', desc: 'Average cost savings per building from optimized operations' },
              ].map((benefit, i) => (
                <div
                  key={benefit.title}
                  className={`animate-on-scroll animate-scale stagger-${i + 1} bg-gradient-to-br from-gray-50 to-white border border-gray-100 rounded-2xl p-8 text-center relative overflow-hidden transition-all hover:-translate-y-2 hover:shadow-2xl group`}
                >
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[var(--primary)] to-cyan-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-400" />
                  <div className="text-5xl font-black mb-2 bg-gradient-to-r from-[var(--primary)] to-cyan-500 bg-clip-text text-transparent">
                    {benefit.stat}
                  </div>
        ...
        {/* SECTION 12: ABOUT */}
        <section className="py-24 px-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0a1929 0%, #0d2137 100%)' }}>
